...
function showTriggerConflictWarning() {
  const triggerKeys = atom.config.get(
    `atom-ide-hyperclick.${process.platform}TriggerKeys`
  );
...
function getTriggerDescription(trigger) {
  const schema = atom.config.getSchema(
    `atom-ide-hyperclick.${process.platform}TriggerKeys`
  );

  if (!(schema != null && schema.enum != null)) {
    throw new Error(
      'Invariant violation: "schema != null && schema.enum != null"'
    );
  }
  ...
  const match = schema.enum.find((option) => {
    if (!(option != null && typeof option.value === "string")) {
      throw new Error(
        "Invariant violation: \"option != null && typeof option.value === 'string'\""
      );
    }
  ...
  if (!(match != null && typeof match.description === "string")) {
    throw new Error(
      "Invariant violation: \"match != null && typeof match.description === 'string'\""
    );
  }
