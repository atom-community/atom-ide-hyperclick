...
class HyperclickForTextEditor {
    ...
    return this._fetchStream.pipe(
      ...
      RxOp.tap((position) => {
        if (position == null) {
          ...
          const match = Range.wordAtPosition(this._textEditor, position)
  ...
  _isInLastSuggestion(position) {
    ...
    return Range.isPositionInRange(position, range)
  }
  ...
  _isInLastWordRange(position) {
    ...
    return Range.isPositionInRange(position, lastWordRange)
  }
