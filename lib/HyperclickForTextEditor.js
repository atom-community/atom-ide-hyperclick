...
const WARN_ABOUT_TRIGGER_CONFLICT_KEY =
  "atom-ide-hyperclick.warnAboutTriggerConflict";
...
class HyperclickForTextEditor {
  ...
  constructor(textEditor, hyperclick) {
    ...
    this._subscriptions.add(
      atom.commands.add(this._textEditorView, {
        "atom-ide-hyperclick:confirm-cursor": () =>
          this._confirmSuggestionAtCursor(),
      })
    );
    ...
    const updateSuggestionSubscription = this._suggestionStream.subscribe(
      (suggestion) => this._updateSuggestion(suggestion)
    );
    this._subscriptions.add(
      new Disposable(() => {
  ...
  _confirmSuggestion(suggestion) {
    if (Array.isArray(suggestion.callback) && suggestion.callback.length > 0) {
      ...
      if (!(typeof suggestion.callback === "function")) {
        throw new Error(
          "Invariant violation: \"typeof suggestion.callback === 'function'\""
        );
      }
    ...
    return this._fetchStream.pipe(
      ...
      RxOp.tap((position) => {
        if (position == null) {
          ...
          const match = (0, Range.wordAtPosition)(this._textEditor, position);
        ...
        return RxJS.using(
          ...
          () =>
            RxJS.defer(() =>
              ...
              RxOp.startWith(null), // Clear the previous suggestion immediately.
              RxOp.catchError((e) => {
                console.error(
                  "atom-ide-hyperclick: Error getting Hyperclick suggestion:",
                  e
                );
                return RxJS.of(null);
              })
  ...
  _getMousePositionAsBufferPosition(mouseEvent) {
    ...
    try {
      ...
      console.error(
        "atom-ide-hyperclick: Error getting buffer position for screen position:",
        error
      );
  ...
  _isInLastSuggestion(position) {
    ...
    return (0, Range.isPositionInRange)(position, range);
  }
  ...
  _isInLastWordRange(position) {
    ...
    return (0, Range.isPositionInRange)(position, lastWordRange);
  }
      ...
      .pipe(
        RxOp.switchMap(() =>
          RxJS.Observable.create(() => {
            ...
            return () => {
              this._textEditorView.classList.remove(
                "atom-ide-hyperclick-loading"
              );
            };
          })
        )
